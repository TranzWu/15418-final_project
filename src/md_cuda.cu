...
void CudaSim::getForces(float *out)
{
    cudaMemcpy(out, forces, 3 * numberOfParticles * sizeof(float), cudaMemcpyDeviceToHost);
}

void CudaSim::getKinetic(float *out)
{
    // Copies kinetic energy from device to out
    cudaMemcpy(out, kinetic, sizeof(float), cudaMemcpyDeviceToHost);
}

void CudaSim::getPotential(float *out)
{
