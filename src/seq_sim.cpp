...
double SeqSim::calculateDistance(Particle p1, Particle p2){
    double distance = 0;
    for (int i = 0; i < 3; i++){
...
void SeqSim::updateBondAndAngle()
{
    double spring = 1000;
    double x0 = 0.8;
...
void SeqSim::calculateForceAndEnergy()
{
    potential = 0;
    initializeForce();
...
SeqSim::SeqSim(size_t number_of_particles_in, float box_size, float dudr,
...
void SeqSim::getPositions(float *out)
{
    for (int i = 0; i < number_of_particles; i++)
    {
...
void SeqSim::getVelocities(float *out)
{
    for (int i = 0; i < number_of_particles; i++)
    {
...
float SeqSim::getKinetic()
{
    return kinetic;
}

float SeqSim::getPotential()
{
    return potential;
}
